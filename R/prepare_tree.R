...
prepare_tree <- function(tree_res,
  ...
  if (model == "party") {
    plot_data <- plot_data %>% mutate(
      parent_label = paste0(
        splitvar, " (", round(recalculated_nodesize/total_n*100, 0), "%)\n",
  ...
  if (model == "rpart") {
    ...
    plot_data <- plot_data %>% rowwise() %>% mutate(
      prob_text = paste(round(c_across(all_of(prob_cols)), 2), collapse = ", "),
      parent_label = paste0(
        splitvar, " (", round(recalculated_nodesize/total_n*100, 0), "%)\n",
        "(", prob_text, ")"
      ),
    ...
    ) %>% ungroup() %>% dplyr::select(-prob_text)
  }


  plot_data <- plot_data %>% mutate(
    node_label = if_else(kids > 0, parent_label, child_label)
  )

  compute_branches <- function(pd) {
    children <- pd %>% filter(!is.na(parent)) %>%
      dplyr::select(child_id = id, parent)
    parents <- pd %>% dplyr::select(parent_id = id, x_parent = x, y_parent = y)
    segs <- children %>%
      left_join(parents, by = c("parent" = "parent_id")) %>%
      left_join(pd %>% dplyr::select(child_id = id, x_child = x, y_child = y),
                by = "child_id")
    segs %>% transmute(
      x_start = x_parent, y_start = y_parent,
      x_end   = x_child,  y_end   = y_child
    )
  }
